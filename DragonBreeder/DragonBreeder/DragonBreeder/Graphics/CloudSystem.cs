...
{
    public class SkyCloudSystem : GraphicObject
    {
        ...
        public SkyCloudSystem(int MaxCount)
        {
            ...
            Bounds = new Vector2(500, 500);
            OriginPoint = new Vector2(-500, -500);
            MaxHeight = 20.0f;
            MinHeight = 15.0f;
            CloudTextureCount = 3;
            ...
            for (int i = 0; i < clouds.Count; i++)
            {
                ...
                vertices[i].TextureCoordinate = new Vector2(cloudIDs[i], 10*(float)rand.NextDouble());
        ...
        {
            SkyDraw = ContentManager.Load<Effect>("Engine/CloudySky");
            ...
            Texture2D lolTest = ContentManager.Load<Texture2D>("sun");
            CloudTextures.AddOne(lolTest);
            mask = new RenderTarget2D(GraphicsDevice, 1280, 720, true, SurfaceFormat.Color, DepthFormat.Depth16);
        }
        public void DrawClouds(GraphicsDevice gd, RenderTarget2D DepthBuffer, Camera Camera)
        {
            SkyDraw.CurrentTechnique = SkyDraw.Techniques["RenderCloudsPost"];
            ...
            gd.Vertices[0].SetSource(vbuf, 0, vdecl.GetVertexStrideSize());
            SkyDraw.Begin();
            SkyDraw.CurrentTechnique.Passes[0].Begin();
            gd.DrawPrimitives(PrimitiveType.PointList, 0, clouds.Count);
        ...
        public void PrepareParamsSkyBox(RenderTarget2D Depth, Camera Camera)
        {
            ...
            SkyDraw.Parameters["Resolution"].SetValue(Resolution);
        }
