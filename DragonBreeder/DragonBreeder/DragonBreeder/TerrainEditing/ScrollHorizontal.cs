...
{
    class ScrollHorizontal : Lockable, GUIElement
    {
        ...
        public int ContentWidth
        {
            ...
            {
                contentWidth = value;
                scrollerRegion.Width = contentWidth - scrollRegion.Width;
                scrollerRegion.Width = scrollRegion.Width - scrollerRegion.Width;
                if (scrollerRegion.Width < 0)
                {
                    scrollerRegion.Width = scrollRegion.Width;
                }
            }
        }
        ...
        public Vector2 Dimmensions
        {
            get
            {
                return new Vector2(scrollRegion.Width, scrollRegion.Height);
            }
            set
            {
                scrollRegion.Width = (int)value.X;
                scrollRegion.Height = (int)value.Y;
            }
        }
        public float ValueNormalized
        {
            get
            {
                float a = scrollRegion.X;
                float b = scrollRegion.Width - scrollerRegion.Width;
                float s = scrollerRegion.X;
                b -= a;
                s -= a;
                s /= b;
                return s;
        ...
        public ScrollHorizontal(Texture2D Scroll, Texture2D Scroller, int Length, int Width)
        {
            this.scroll = Scroll;
            this.scroller = Scroller;
            scrollRegion = new Rectangle(0, 0, Length, Width);
            scrollerRegion = new Rectangle(0, 0, Length, Width);
            contentWidth = -1;
            ScrollerPosition = 0;
        }
        int ClickPos = 0;
        public void Update()
        {
            if ((Lock == true && scrolling == true) || (Lock == false && scrolling == false) )
            {
                ...
                if ((scrollerRegion.Contains(state.X, state.Y) || scrolling == true) && state.LeftButton == ButtonState.Pressed)
                {
                    ...
                    Lock = true;
                    scrolling = true;
                    int newPos = (int)MathHelper.Clamp((state.X-ClickPos), scrollRegion.X, (float)(scrollRegion.Width-scrollerRegion.Width));
                    scrollerRegion.X = newPos;
                }
                else
                {
                    Lock = false;
