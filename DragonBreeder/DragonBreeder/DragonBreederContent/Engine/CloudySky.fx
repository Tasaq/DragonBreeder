...
VS_OUT VS( VS_IN input )
{
    ...
    output.pos =input.Position;// mul(float4(input.Position.xyz,1), World);
...
float4 PS( VS_OUT input ) : SV_TARGET
{
	...
	if(d<=0)
	{
		float3 color = lerp(float3(0.5,0.55,1.0),float3(0.4,0.45,0.8f),input.pos.y);
...
float3 LightPosition = float3(100,40,5);
...
float4 PS_FS( VS_OUT input ) : SV_TARGET
{
	...
	float2 prop = Resolution.x/Resolution.y;
	prop.y = 1.0f;
	prop *= 0.9f;
	...
	float dts = distance(lightScreenPos*prop, input.texcoord*prop);
