...
DS_OUTPUT DS_PNtrianglesDisplace(HS_PATCH_DATA patchData, const OutputPatch<HS_CONTROL_POINT, 3> input, float3 uvw : SV_DOMAINLOCATION)
{
	...
	output.Position = float4(final_position,1);
	return output;		
}
[maxvertexcount(12)]
...
{
	VS_OUT output;
	float4 v0 = mul(input[0].Position, ViewProjection);
	float4 v1 = mul(input[1].Position, ViewProjection);
	float4 v2 = mul(input[2].Position, ViewProjection);
...
{
	GS_OUT output;
	float4 v0 = mul(input[0].Position, World);
	float4 v1 = mul(input[1].Position, World);
	float4 v2 = mul(input[2].Position, World);
	...
	for(uint i =0; i<3;i++)
	{
		GS_OUT input2;
		input2.RTIndex = i;
		v0 = mul(input[0].Position, ViewProjectionSplits[i]);
		v1 = mul(input[1].Position, ViewProjectionSplits[i]);
		v2 = mul(input[2].Position, ViewProjectionSplits[i]);
		input2.Position = v0;
		input2.Normal	= n0;
		input2.TexCoord = t0;
		input2.pos = input2.Position;
...
float PS_Depth_old( GS_OUT input ) : SV_Target
{
