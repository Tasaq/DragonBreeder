...
HS_PATCH_DATA HS_PatchConstant(OutputPatch<HS_CONTROL_POINT, 3> controlPoints, uint pid : SV_PrimitiveID)
{
	...
	//float culling = dot(normal, normalize(CameraDirection));
	float2 cp1 = mul(float4(controlPoints[0].pos.xyz,1), World).xz;
	float2 cp2 = mul(float4(controlPoints[1].pos.xyz,1), World).xz;
	float2 cp3 = mul(float4(controlPoints[2].pos.xyz,1), World).xz;
	
	float tessCoeff = 50;
	if (cp1.x == cp2.x)
	{
		float d1 = distance(CameraPosition.xz, (cp1+cp2)/2.0f);
		d1 = (1-saturate(d1/tessCoeff))*TessellationFactor+1.0f;
		float d2 = distance(CameraPosition.xz, (cp2+cp3)/2.0f);
		d2 = (1-saturate(d2/tessCoeff))*TessellationFactor+1.0f;
		float d3 = distance(CameraPosition.xz, (cp1+cp3)/2.0f);
		d3 = (1-saturate(d3/tessCoeff))*TessellationFactor+1.0f;
		patch.edge[0] = d3; // przeciwprostokatna
		patch.edge[1] = d1;//lewy
		patch.edge[2] = d2;//dolny
		patch.inside = 5;( patch.edge[0] + patch.edge[1] + patch.edge[2] ) / 3.0f;
	}
	else
	{
		float d1 = distance(CameraPosition.xz, (cp1+cp2)/2.0f);
		...
		float d2 = distance(CameraPosition.xz, (cp2+cp3)/2.0f);
		d2 = (1-saturate(d2/tessCoeff))*TessellationFactor+1.0f;
		float d3 = distance(CameraPosition.xz, (cp3+cp1)/2.0f);
		d3 = (1-saturate(d3/tessCoeff))*TessellationFactor+1.0f;
		patch.edge[0] = d3;//d1;//przeciwprostokatna
		patch.edge[1] = d1;//gorny
		patch.edge[2] = d2;//lewy
		patch.inside = 5;( patch.edge[0] + patch.edge[1] + patch.edge[2] ) / 3.0f;
	}
...
PS_OUT PS_Textured( VS_OUT input ) : SV_TARGET
{
	...
	//output.Color.rgb = float3(input.TexCoord.rg,0);
	output.Color.a = 1;
	return output;
}
